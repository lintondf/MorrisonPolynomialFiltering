...
RealMatrix runCase(long order, double tau, long N, RealMatrix data ) {
	std::shared_ptr<Components::FMPBase> fmp = Components::makeFMP(order, tau);
	...
	for (int i = 1; i < N; i++) {
		...
		actual.block(i, 0, 1, order + 1) = fmp->getState(fmp->getTime()).transpose();
	}
	return actual;
}

TEST_CASE("Fading Memory Polynomial Filter Test") {
	INFO("FMP TEST");
	SUBCASE("Perfect Test") {
		std::shared_ptr <TestData> testData = TestData::make(filename);
		std::vector<std::string> matches = testData->getMatchingGroups("testFMPPerfect");
	...
	SUBCASE("Full Suite Test") {
		...
		std::shared_ptr <TestData> testData = TestData::make(filename);
		std::vector<std::string> matches = testData->getMatchingGroups("FMPFullSuite");
		for (int i = 0; i < matches.size(); i++) {
			RealMatrix setup = testData->getGroupVariable(matches.at(i), "setup");
			RealMatrix data = testData->getGroupVariable(matches.at(i), "data");
			RealMatrix expected = testData->getGroupVariable(matches.at(i), "expected");
			...
			//fmp->start(data(0, 0), data.block(0, 2, 1, order+1).transpose());
			//for (int i = 1; i < N; i++) {
			//	RealVector Z = fmp->predict(data(i, 0));
			//	double e = data(i, 1) - Z(0);
			//	fmp->update(data(i, 0), Z, e);
