...
#                 print(i, Yf, Yf[0]-truth[i,0])
        ...
        for i in range(0,N) :
            ...
            Yf = emp.getState(times[i][0])
            r = array2string(Yf, formatter={'float_kind':lambda y: "%10.4g" % y})
            print("%5d %10.4g %s %10.4g" % (i, times[i][0], r, Yf[0]-truth[i,0]))
...
#         fmp.initialize(0.0, Y0, tau)
        fmp.setTau(tau)
        for i in range(0,N) :
            fmp.add(times[i][0], observations[i,0])
            Z = fmp.getState(times[i])
            r = array2string((Z-truth[i,:])/truth[i,:], formatter={'float_kind':lambda y: "%10.4g" % y})

            print("%10.4g %10.4g %s %10.4g" % (times[i][0], truth[i,0], r, Z[0] - truth[i,0]))
