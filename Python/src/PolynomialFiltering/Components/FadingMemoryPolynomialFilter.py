...
class FMP3(FMPBase):    
    ...
    def _VRF(self) -> array:
        ...
        d[1] = 5*(53*t**4 + 298*t**3 + 762*t**2 + 970*t + 581)*(1-t)**3 / (18*u**2*(1+t)**7)
...
#         return 7.7478/(1.0-self.theta)
    ...
    def _VRF(self) -> array:
        ...
        d[1] = 7*(17467*t**8+124874*t**7+478036*t**6+1239958*t**5+2345510*t**4+3250918*t**3+3352636*t**2+2454074*t+1028527)*(1-t)**3 / (1800*u**2*(1+t)**11)
        d[2] = 7*(7121*t**6+43016*t**5+129715*t**4+244880*t**3+295855*t**2+225176*t+87581)*(1-t)**5 / (72*u**4*(1+t)**11)
        d[3] = 3*(2549*t**4+12072*t**3+24926*t**2+25176*t+11117)*(1-t)**7 / (4*u**6*(1+t)**11)
        d[4] = 14*(113*t**2+316*t+221)*(1-t)**9 / (u**8*(1+t)**11)
        d[5] = (252*(1-t)**11) / (u**10*(1+t)**11)        
        D = diag( sqrt(d) );
        return D @ K @ D;
